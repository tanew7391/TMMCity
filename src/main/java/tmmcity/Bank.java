...
public class Bank {
    ...
    public void monthEnd() { // method which calculates total money at months end by adding revenue,
                             // deducting expenses and loans
        System.out.println("ResidentialTaxes: " + residentialTaxes + ", Tax: " + tax + ", revenue: " + revenue
                + ", increase: " + ((revenue * tax) + (residentialTaxes * tax)) + ", Decrease: "
                + (expense + smallLoan + mediumLoan + bigLoan));
        money += (revenue * tax) + (residentialTaxes * tax) - expense - smallLoan - mediumLoan - bigLoan;

        // if statements which reduce one month off of each term if there is more than 0
        // months remaining
        if (bigTerm > 0) {
            bigTerm -= 1;
        }
    ...
    public String toString() { // to string method
        ...
        message += "Current tax rate: " + moneyformat.format(tax) + "\n"
                + "Current money being generated from taxes per month: "
                + moneyformat.format((revenue * tax) + (residentialTaxes * tax)) + "\n" + "Current revenue "
                + moneyformat.format(revenue) + "\n" + "Current montly loan payment: "
                + moneyformat.format((bigLoan + mediumLoan + smallLoan)) + "\n" + "Total loan payment outstanding: "
                + moneyformat.format((((bigLoan * bigTerm) + (mediumLoan * mediumTerm) + (smallLoan * smallTerm))))
